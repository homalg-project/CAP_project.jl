...
uninstall:
	julia -e 'using Pkg; Pkg.rm("AdditiveClosuresForCAP");'

test:
	julia -e 'using Pkg; Pkg.test("AdditiveClosuresForCAP");'
...
git-commit:
	...
	echo -e "\033[32mCommitting changes in \"AdditiveClosuresForCAP\"\033[0m"; \
	git add .; \
	git commit \
		...
		echo -e "\033[33mNo changes in \"AdditiveClosuresForCAP\" to commit or no version bump detected.\033[0m"; \
	fi
