...
end; CategoryFilter = cat -> HasIsLinearCategoryOverCommutativeSemiring( cat ) && IsLinearCategoryOverCommutativeSemiring( cat ) && HasCommutativeSemiringOfLinearCategory( cat ) );
...
AddDerivationToCAP( IsomorphismFromImageObjectToKernelOfCokernel,
  ...
  function( cat, morphism )
    ...
    return UniversalMorphismFromImage( cat, morphism, PairGAP( morphism_to_kernel, kernel_emb ) );
...
[
  ...
  CategoryFilter =
    function( cat )
      ...
      if (@not IsIdenticalObj( CommutativeSemiringOfLinearCategory( cat ), CommutativeSemiringOfLinearCategory( range_cat ) ))
...
]; CategoryFilter = cat -> @IsBound( cat.supports_empty_limits ) && cat.supports_empty_limits == true );
